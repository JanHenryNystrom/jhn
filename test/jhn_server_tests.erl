...
run_simple(cast) ->
    ...
    ?assertEqual(ok, a_jhn_server:cast(Pid, {reply, bb})),
    ?assertEqual({reply, bb}, wait()),
        ?assertEqual(ok, a_jhn_server:cast({testServer, node()}, {reply, cc})),
...
run_simple(abcast) ->
    ?assertEqual(ok, a_jhn_server:abcast(testServer, {reply, aa})),
    ?assertEqual({reply, aa}, wait()),
    ?assertEqual(ok, a_jhn_server:abcast([node()], testServer, {reply, aa})),
    ?assertEqual({reply, aa}, wait()),
    ?assertEqual(ok,
                 a_jhn_server:abcast([node(), node()], testServer,{reply, aa})),
    ?assertEqual({reply, aa}, wait()),
    ?assertEqual({reply, aa}, wait()),
...
run_simple(reply) ->
    ?assertError({not_behaviour_context, jhn_server}, jhn_server:reply(broken)),
...
run_simple(from) ->
    ?assertError({not_behaviour_context, jhn_server}, jhn_server:from()),
    erlang:put('$jhn_behaviour', none),
    ...
    ?assertEqual(ok, a_jhn_server:cast(testServer, {ereply, ff})),
...
run_simple(hibernate) ->
    ?assertEqual(ok, a_jhn_server:cast(testServer, hibernate)),
    ?assertEqual(hibernate, wait()),
    ?assertEqual({reply, ff}, a_jhn_server:call(testServer, {reply, ff})),
    ?assertEqual(ok, a_jhn_server:cast(testServer, hibernate)),
    ?assertEqual(hibernate, wait()),
    ?assertEqual(ok, a_jhn_server:cast(testServer, {reply, ff})),
    ?assertEqual({reply, ff}, wait()),
...
run_error(multireply) ->
    ...
    ?assertEqual(true, lists:member(Pid, Links)),
    Flag = process_flag(trap_exit, true),
    ?assertEqual({reply, aa}, a_jhn_server:call(testServer, {rreply, aa})),
