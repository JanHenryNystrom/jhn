...
run_simple(event) ->
    Pid = whereis(testFsm),
    ?assertEqual(ok, a_jhn_fsm:event(testFsm, {reply, aa})),
    ?assertEqual({reply, aa}, wait()),
    ?assertEqual(ok, a_jhn_fsm:event(Pid, {reply, bb})),
    ?assertEqual({reply, bb}, wait()),
        ?assertEqual(ok, a_jhn_fsm:event({testFsm, node()}, {reply, cc})),
...
run_simple(reply) ->
    ?assertError({not_behaviour_context, jhn_fsm}, jhn_fsm:reply(broken)),
    erlang:put('$jhn_behaviour', none),
    ?assertError({wrong_behaviour_context,jhn_server},jhn_server:reply(broken)),
    erlang:put('$jhn_behaviour', undefined);
run_simple(from) ->
    ?assertError({not_behaviour_context, jhn_fsm}, jhn_fsm:from()),
    erlang:put('$jhn_behaviour', none),
    ?assertError({wrong_behaviour_context, jhn_server}, jhn_server:from()),
    erlang:put('$jhn_behaviour', undefined),
    ?assertEqual({reply, ff}, a_jhn_fsm:call(testFsm, {ereply, ff})),
    ?assertEqual(ok, a_jhn_fsm:event(testFsm, {ereply, ff})),
...
run_simple(hibernate) ->
    ?assertEqual(ok, a_jhn_fsm:event(testFsm, hibernate)),
    ?assertEqual(hibernate, wait()),
    ?assertEqual({reply, ff}, a_jhn_fsm:call(testFsm, {reply, ff})),
    ?assertEqual(ok, a_jhn_fsm:event(testFsm, hibernate)),
    ?assertEqual(hibernate, wait()),
    ?assertEqual(ok, a_jhn_fsm:event(testFsm, {reply, ff})),
    ?assertEqual({reply, ff}, wait()),
...
run_startstop(started) ->
    ...
    Result = a_jhn_fsm:start_link(testFsm, simple, tester, node()),
    ?assertEqual({error, {already_started, testFsm, self()}}, Result),
    Result1 = a_jhn_fsm:start_link(testFsm, hibernate, tester, node()),
    ?assertEqual({error, {already_started, testFsm, self()}}, Result1),
    unregister(testFsm),
    register(tester, self());
...
error_test_() ->
    {setup,
     fun setup_error/0,
     fun cleanup_error/1,
     {inorder, [{timeout, 120, {"Multiple Replies",
                                ?_test(run_error(multireply))}},
                {timeout, 120, {"Badeturn(call)",
...
run_error(multireply) ->
    ...
    ?assertEqual(true, lists:member(Pid, Links)),
    Flag = process_flag(trap_exit, true),
    ?assertEqual({reply, aa}, a_jhn_fsm:call(testFsm, {rreply, aa})),
    ?assertEqual({terminate, {multiple_replies, {rreply, aa}}}, wait()),
    ?assertEqual({'EXIT', Pid, {multiple_replies, {rreply, aa}}}, wait()),
    process_flag(trap_exit, Flag);
...
run_error(eventnofsm) ->
    ?assertEqual(ok, jhn_fsm:event(does_not_exist, hello)).
...
run_coverage(format_status) ->
    ...
    TheState =
        {state, the_parent,
         ...
         {[], []},
         false},
    jhn_fsm:format_status(none, [PDict, sysState, parent, debug, TheState]),
    TheState1 =
        {state,
         the_parent,
         self(),
         ...
         {[], []},
         false},
    jhn_fsm:format_status(none, [PDict, sysState, parent, debug, TheState1]),
    TheState2 =
        {state,
         the_parent,
         self(),
...
run_states(hibernate) ->
    ...
    ?assertEqual(ok, b_jhn_fsm:event(b_jhn_fsm, {goto, third})),
    ?assertEqual(third, b_jhn_fsm:call(b_jhn_fsm, get_state)),
    ?assertEqual(ok, b_jhn_fsm:event(b_jhn_fsm, hibernate)),
    ?assertEqual(hibernate, wait()),
    ?assertEqual(ok, b_jhn_fsm:event(b_jhn_fsm, {goto, second})),
    ?assertEqual(second, b_jhn_fsm:call(b_jhn_fsm, get_state)),
    b_jhn_fsm ! hibernate,
    ...
    ?assertEqual(ok, b_jhn_fsm:event(b_jhn_fsm, {goto, first})),
    ?assertEqual(first, b_jhn_fsm:call(b_jhn_fsm, get_state)),
    ?assertEqual(ok, b_jhn_fsm:event(b_jhn_fsm, event_hibernate)),
    ?assertEqual(hibernate, wait()),
    ?assertEqual(ok, b_jhn_fsm:event(b_jhn_fsm, {goto, second})),
    ?assertEqual(second, b_jhn_fsm:call(b_jhn_fsm, get_state)),
    ?assertEqual(ok, b_jhn_fsm:event(b_jhn_fsm, {goto, first}));
